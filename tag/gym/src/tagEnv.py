...
class TagEnv(BaseEnv):
    ...
    def __init__(self, args=None, cfg: TagConfig = TagConfig()):
        ...
        # Init
        gs.init(logging_level=args.logging_level if args is not None else "info", backend=self.device)
    ...
    def build(self):
        self.scene.build(n_envs=self.n_envs, env_spacing=self.cfg.vis.env_spacing if self.n_rendered > 1 else [0, 0])
        if self.cfg.vis.visualized:
    ...
    def get_privileged_obs(self) -> Union[torch.Tensor, Dict]:
        """Get privileged observation buffer data

        Returns:
            Union[torch.Tensor, None]: A Tuple of the Privileged Observation Buffer and any Extras
        """
